...
import { useLogPolling } from '@/hooks/useHistoricalLogs';
import type { LogLevel } from '@/types/logs';
import { Toaster, toast } from 'sonner';
import { getInitialFilters, LAST_PRESET_KEY, loadLastPresetId } from '@/lib/presets';
...
export default function App() {
  ...
  const { logs, connected, error, loadOlder, refresh, hasMore, isLoading, totalCount } = useLogPolling({
    selectedLevels,
    targetFilter,
    searchFilter,
  });
  ...
  const handleClearFilters = () => {
    setSelectedLevels(new Set(['trace', 'debug', 'info', 'warn', 'error']));
    setTargetFilter('');
    setSearchFilter('');
    // Save default preset as the last selected
    localStorage.setItem(LAST_PRESET_KEY, '__default__');
    setLastPresetId('__default__');
  };
      ...
      <header className="bg-card border-b border-border px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h1 className="text-2xl font-bold text-foreground">Tracing Dashboard</h1>
        ...
        <div className="flex-1 flex flex-col min-w-0 relative">
          ...
          <div className="px-4 py-3 bg-muted/30 border-b border-border space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              Showing {logs.length.toLocaleString()} of {totalCount.toLocaleString()} logs
          ...
          <div className="flex-1 overflow-hidden">
            <LogViewer
              key={reverseOrder ? 'reversed' : 'normal'}
              logs={logs}
